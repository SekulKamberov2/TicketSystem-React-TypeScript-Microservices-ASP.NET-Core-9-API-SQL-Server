...
const RoundedResetButton = styled.button<StyledButtonProps>`
  padding: 5px 8px;
...
const ProfilePage: React.FC = () => {
  ...
  const EMPLOYEE = user?.roles?.some((role) => role === 'EMPLOYEE');
  ...
  return (
    <PageContainer>
      ...
      <>
        <ProfileHeader>
          {user.roles ? user.roles.join(' & ') : 'Not Employee'}
        </ProfileHeader>
        <UserNameHeader>{user.userName}</UserNameHeader>
        {EMPLOYEE && (
          <RoundedResetButton width="111px" onClick={openModal}>
            Reset Pass
          </RoundedResetButton>
        )}
        <UserCard>
          ...
          <span>
            <strong>Created: </strong> {new Date(user.dateCreated).toLocaleString()}
          </span>
          <span>
